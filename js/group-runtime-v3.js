(() => {
  ...
  async function acceptPendingEmailInvite(user) {
    if (!user?.email || state.acceptingInvite) return false;
  ...
  async function resolveContext(force = false) {
    ...
    if (!force && state.contextPromise) return state.contextPromise;

    state.contextPromise = (async () => {
      await acceptPendingEmailInvite(user);
      ...
      if (reverseLink.groupId) accessibleIds.add(reverseLink.groupId);
      ...
      if (!isOwner) {
        await Promise.all(allGroups.map(async group => {
          ...
          try {
            ...
            // La lecture des groupes non attribués est volontairement refusée.
          }
        }));
      }
      ...
      if (!isOwner && groups.length) {
        ...
        groups.forEach(group => {
          if (membership.groupIds?.[group.id] !== true) {
            repairs[`organizationMembers/${organizationId}/${user.uid}/groupIds/${group.id}`] = true;
          }
          if (reverseLink.groupIds?.[group.id] !== true) {
            repairs[`userOrganizations/${user.uid}/${organizationId}/groupIds/${group.id}`] = true;
          }
        });
        if (Object.keys(repairs).length) {
          db.ref().update(repairs).catch(error => {
            console.warn('Synchronisation des index de groupes impossible :', error);
          });
        }
      }
      ...
      const context = {
        ...
        groups,
        isOwner,
        isEnterpriseMember: organization.plan === 'enterprise' ||
  ...
  async function selectedGroupContext() {
    ...
    if (!groupId) groupId = context.groups[0].id;
  ...
  async function repairOwnGroupSessions() {
    ...
    try {
      ...
      snapshots.forEach((snapshot, index) => {
        ...
        if ((!groupId || !organizationId) && !context.isOwner && context.groups.length === 1) {
  ...
  async function assignSession(code, groupId) {
    ...
    updates[`organizerSessions/${session.ownerUid || user.uid}/${code}/organizationId`] = context.organizationId;
    updates[`organizerSessions/${session.ownerUid || user.uid}/${code}/groupId`] = group.id;
    updates[`organizerSessions/${session.ownerUid || user.uid}/${code}/groupName`] = group.name || 'Groupe';
  ...
  auth.onAuthStateChanged(async user => {
    ...
    await acceptPendingEmailInvite(state.user);
