...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        await loadQuizModule('js/organizer-auth.js?v=14');
        await loadQuizModule('js/google-auth-fix.js?v=14');
        await loadQuizModule('js/platform-features.js?v=14');

        if (document.body.classList.contains('home-page')) {
            await loadQuizModule('js/home-role-flow.js?v=14');
        }

        if (page === 'player' || document.body.classList.contains('home-page')) {
            await loadQuizModule('js/player-session-auth.js?v=14');
            await loadQuizModule('js/participant-guard.js?v=14');
        }
        if (page === 'player' || page === 'admin') {
            await loadQuizModule('js/speed-scoring.js?v=14');
        }
        if (page === 'admin') {
            await loadQuizModule('js/participant-guard.js?v=14');
            await loadQuizModule('js/pptx-import.js?v=14');
        }
    } catch (error) {
