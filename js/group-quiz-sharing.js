(() => {
  ...
  async function loadContext(force = false) {
    ...
    contextPromise = (async () => {
      ...
      const accessibleGroupIds = new Set(
        Object.keys(membership.groupIds || {}).filter(groupId => membership.groupIds[groupId] === true)
      );

      const linkedOrganization = links[organizationId] || {};

      if (linkedOrganization.groupId) {
        accessibleGroupIds.add(linkedOrganization.groupId);
      }

      Object.keys(linkedOrganization.groupIds || {}).forEach(groupId => {
        if (linkedOrganization.groupIds[groupId] === true) {
  ...
  async function injectCreateGroupSelector() {
    ...
    if (!form || form.querySelector('#quizGroupId')) {
      return;
    }
    ...
    try {
      ...
      box.innerHTML = `
        ...
        <small style="display:block;margin-top:7px;opacity:.72">
          Tous les membres de ce groupe verront et pourront ouvrir ce quiz.
        </small>
      `;
  ...
  async function loadGroupSessions() {
    ...
    const uniqueSessions = Array.from(
      new Map(sessions.map(session => [session.code, session])).values()
    );
  ...
  async function assignSession(code, groupId) {
    ...
    if (!group) {
      throw new Error('Groupe inaccessible.');
    }
    ...
    if (session.ownerUid !== currentUser.uid && !context.isOwner) {
  ...
  async function removeAssignment(code) {
    ...
    if (!session || session.ownerUid !== currentUser.uid) {
  ...
  async function renderDashboardGroupQuizzes() {
    if (rendering || document.body?.dataset?.page !== 'dashboard') {
      return;
    }
    ...
    if (!title || !content || title.textContent.trim() !== 'Mes quiz') {
      return;
    }
    ...
    try {
      ...
      const personalRows = Array.from(content.querySelectorAll('.dashboard-row')).filter(row =>
        row.querySelector('a[href*="admin.html?code="]')
      );
      ...
      assignment.innerHTML = context.isOwner
        ? `
          ...
          <p class="dashboard-muted">
            Choisis le groupe qui pourra voir et utiliser chaque quiz.
          </p>
          ${
            personalRows.length
              ? personalRows.map(row => {
                  ...
                  return `
                    <div class="dashboard-row" data-group-assign-row="${esc(code)}">
                      <div>
                        <strong>${esc(name)}</strong>
                        <div class="dashboard-muted">${esc(code)}</div>
                      </div>

                      <div style="display:flex;gap:8px;flex-wrap:wrap">
                        <select data-assign-code="${esc(code)}">
                        ...
                        <button type="button" class="btn-primary" data-apply-group="${esc(code)}">
                          Attribuer
                        </button>

                        <button type="button" data-remove-group="${esc(code)}">
                          Privé
                        </button>
                      </div>
                    </div>
              ...
              : '<p class="dashboard-muted">Aucun quiz personnel à attribuer.</p>'
          }
        `
        : '';
      ...
      shared.innerHTML = `
        <div class="dashboard-row">
          <div>
            ...
            <p class="dashboard-muted">
              Tous les membres du groupe voient les mêmes quiz,
              quel que soit leur créateur.
            </p>
          </div>

          <a class="btn-primary" href="index.html?create=1">
            + Créer un quiz
          </a>
        </div>
                ...
                <div class="dashboard-row">
                  <div>
                    ...
                    <div class="dashboard-muted">
                      ${esc(session.groupName)}
                      · ${esc(session.code)}
                      · créé par ${esc(session.ownerEmail || 'un membre')}
                    </div>
                  </div>

                  <a class="btn-primary" href="admin.html?code=${encodeURIComponent(session.code)}">
                    Ouvrir
                  </a>
                </div>
              `).join('')
      ...
      assignment.querySelectorAll('[data-apply-group]').forEach(button => {
        button.onclick = async () => {
          ...
          const selector = assignment.querySelector(
            `[data-assign-code="${CSS.escape(code)}"]`
          );

          if (!selector?.value) {
            notify('Choisis un groupe.', 'error');
            return;
          }
          ...
          try {
            await assignSession(code, selector.value);
  ...
  const observer = new MutationObserver(() => {
    setTimeout(renderDashboardGroupQuizzes, 30);
  });
  ...
  firebase.auth().onAuthStateChanged(user => {
    ...
    if (document.body?.dataset?.page === 'dashboard') {
      observer.observe(document.getElementById('dashboardContent') || document.body, {
        childList: true,
        subtree: true
      });
  ...
  document.addEventListener('click', event => {
    if (event.target.closest('[data-section="quizzes"]')) {
      setTimeout(renderDashboardGroupQuizzes, 120);
    }
  });
