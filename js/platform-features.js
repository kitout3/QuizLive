...
(() => {
  ...
  async function exportHistoryCsv() {
    ...
    sessions.forEach(session => {
      const date = session.createdAt ? new Date(Number(session.createdAt)).toLocaleString('fr-FR') : '';
      lines.push([session.code, session.name || '', date, session.status || '']);
    });
  ...
  async function saveQuestion(question) {
    ...
    await database.ref(`questionBanks/${organizer.user.uid}/${id}`).set({
      id,
      text,
  ...
  async function showDashboard() {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `<div class="modal-content" style="max-width:1050px;max-height:90vh;overflow:auto">
        <button class="modal-close" onclick="document.getElementById('quizPlatformDashboard').classList.remove('active')">&times;</button>
        <h2>📊 Espace organisateur</h2>
        <div id="platformDashboardBody"><p>Chargement...</p></div>
      </div>`;
    ...
    body.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px;margin:18px 0">
        <div class="action-card"><strong>${stats.sessions}</strong><p>Quiz créés</p></div>
        <div class="action-card"><strong>${stats.participants}</strong><p>Participants</p></div>
        <div class="action-card"><strong>${stats.answers}</strong><p>Réponses</p></div>
        <div class="action-card"><strong>${stats.successRate}%</strong><p>Taux de réussite</p></div>
      </div>
      <h3>Organisation</h3>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">
        ${organizations.length ? organizations.map(org => `<span class="status-badge">${TYPES[org.type] || org.type} · ${org.name} · ${PLANS[org.plan || 'free'].name}</span>`).join('') : '<span style="color:var(--text-muted)">Aucune organisation</span>'}
        <button id="createOrganizationBtn" class="btn-primary">+ Créer une organisation</button>
      </div>
      <h3>Historique des quiz</h3>
      <div style="display:flex;justify-content:flex-end;margin-bottom:10px"><button id="exportHistoryCsvBtn" class="btn-primary">Exporter CSV</button></div>
      <div style="max-height:260px;overflow:auto">
        ${sessions.length ? sessions.map(s => `<div style="display:flex;justify-content:space-between;padding:12px;border-bottom:1px solid rgba(255,255,255,.08)"><span><strong>${s.name || s.code}</strong><br><small>${s.code}</small></span><a href="admin.html?code=${s.code}" class="btn-primary" style="text-decoration:none">Ouvrir</a></div>`).join('') : '<p style="color:var(--text-muted)">Aucun quiz</p>'}
      </div>
      <h3 style="margin-top:22px">Abonnement</h3>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px">
        ${Object.entries(PLANS).map(([key, plan]) => `<div class="action-card"><h3>${plan.name}</h3><p>${plan.participants ? `${plan.participants} participants / session` : 'Capacité sur mesure'}</p><small>${key === 'free' ? 'Actif par défaut' : 'Paiement à connecter'}</small></div>`).join('')}
      </div>`;
    ...
    document.getElementById('createOrganizationBtn').onclick = async () => {
      ...
      try { await createOrganization({ name, type }); showToast('Organisation créée'); showDashboard(); }
      catch (error) { showToast(error.message, 'error'); }
    };
  }

  window.QuizPlatform = { createOrganization, listOrganizations, listSessions, computeStats, exportHistoryCsv, saveQuestion, PLANS };
